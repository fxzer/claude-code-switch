...
class ConfigLoader {
  constructor() {
    this.configPath = path.join(os.homedir(), '.claude', 'ccsw-providers.json');
  }
...
请手动创建配置文件或检查模板文件是否存在。`);
  ...
  async createFromTemplate() {
    try {
      // 获取模板文件路径
      const templatePath = path.join(__dirname, '..', 'ccsw-providers.template.json');
  ...
  validateConfig(config) {
    ...
    // 启动时只验证供应商是否存在，不验证模型和API Key的有效性
    // 这样用户可以进入配置界面修复配置问题
