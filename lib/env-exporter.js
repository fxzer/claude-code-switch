...
class EnvExporter {
   ...
   * 写入 ~/.zshrc 文件
   */
  async writeToGlobalZshrc(envVars) {
    try {
      const sectionStart = '# ==== ccsw start ====';
      const sectionEnd = '# ==== ccsw end ====';

      // 生成配置内容
      let configContent = `${sectionStart}\n`;
      configContent += '# AI 模型配置 - 由 ccsw 命令自动生成\n';
      ...
      configContent += `# 配置时间: ${new Date().toLocaleString('zh-CN')}\n`;
